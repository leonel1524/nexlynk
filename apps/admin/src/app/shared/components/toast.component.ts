  ...
  template: `
    <div class="fixed top-4 right-4 z-50 space-y-2 pointer-events-none">
      @for (toast of toastService.toasts(); track toast.id) {
        <div
          class="pointer-events-auto max-w-sm w-full shadow-lg rounded-lg p-4 flex items-center gap-3 animate-slide-in"
        ...
        >
          ...
          <button
            ...
            class="flex-shrink-0 text-current opacity-50 hover:opacity-100"
          >
...
export class ToastComponent {
  ...
  getClasses(type: string): string {
    const base = 'rounded-lg p-4 flex items-center gap-3';
    switch (type) {
      case 'success':
